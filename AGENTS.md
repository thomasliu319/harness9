...
│   │   ├── web_safety.go            # SSRF 防护（isSafeURL：scheme/userinfo/DNS/IP 段检查）
│   │   ├── web_safety_test.go       # 16 个用例（IP 段 / scheme / DNS fail-closed / IPv6 loopback）
│   │   ├── web_content.go           # HTML→Markdown 管线（go-readability + html-to-markdown + tokenizer fallback）
│   │   ├── web_content_test.go      # 4 个用例（基础转换 / 截断 / fallback / 1MB 限制）
